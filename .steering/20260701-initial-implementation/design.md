...
type User = { key: UserKey; name: string; initial: string; jobTitle: string
  roleColor: string; roleBg: string; avatarBg: string; sfProfile: string }

type Message = { id: string; role: 'user' | 'agent'; content: string; streaming?: boolean }

type FlowStep = { step: 0|1|2|3|4; label: string; status: FlowStatus
  detail?: string; tokenType?: 'entra' | 'cdata' | 'salesforce' }
